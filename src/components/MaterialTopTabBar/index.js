...
export default class TabBarTop extends React.PureComponent<Props> {
  ...
  _renderLabel = ({ route }) => {
    ...
    /*    const fontWeight = position.interpolate({
      inputRange,
      outputRange: inputRange.map(inputIndex => (inputIndex === index ? activeTintFontWeight : inactiveTintFontWeight || '200')),
    }); */
    ...
    if (typeof label === 'string') {
      return (
        <Animated.Text numberOfLines={1} style={[styles.label, { color, fontSize }, labelStyle]} allowFontScaling={allowFontScaling}>
