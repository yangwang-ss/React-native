...
export default class Nine extends React.Component {
  static navigationOptions = ({ navigation }) => ({
    title: navigation.state.params.title || '9.9包邮',
  });
   ...
   */
  // 父级类目商品
  async getNineProduct() {
    let loadingState = 'loading';
    this.setState({ loadingState });
    const { pageSize, sortParams, hasCoupon } = this.state;
    ...
    const res = await getNineProduct(params);
    console.log('getNineProduct===', res);
    if (res && res.length) {
      curPage++;
      this.setState({
        canSwitch: true,
