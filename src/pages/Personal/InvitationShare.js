...
import { StyleSheet, Text, View, Image, TouchableOpacity, ScrollView, StatusBar, Clipboard } from 'react-native';
...
export default class InvitationShare extends Component {
  ...
  drawImg = async (canvas, index) => {
    const { shareImgs, result } = this.state;
    const { backageImage, elements, isSplit, invitation_code, id, height } = result[index];
    canvas.width = 375;
    canvas.height = 667;
    ...
    draw = () => {
      if (isSplit) {
        elements.forEach((item, i) => {
          drawText(ctx, invitation_code[i], invitation_code[i].length, item.font, item.color, item.x, item.y);
          ctx.save();
        });
      } else {
        drawText(ctx, invitation_code, invitation_code.length, elements[0].font, elements[0].color, elements[0].x, elements[0].y);
        ctx.save();
      }
    ...
    image.src = await this.downloadImage(backageImage);
  ...
  render() {
    const { isDraw, showFullPic, fullPicInfo, imgSwiperIndex } = this.state;
    return (
