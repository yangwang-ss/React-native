...
import CollectList from '../../components/CutPriceList';
import { getCutPriceList, deleteCutPriceList } from '../../services/api';
...
export default class Mycollect extends Component {
  ...
  async getList(page) {
    ...
    const res = await getCutPriceList(page);
    console.log("res====",res)
    if (res && res.length) {
      if (res.length > 5) {
        page++;
  ...
  async deleteCollect() {
    ...
    for (let i = 0; i < pList.length; i++) {
      for (let j = 0; j < idList.length; j++) {
        if (pList[i].goodsId === idList[j]) {
          pList.splice(i, 1);
        }
      }
    }
    await deleteCutPriceList(idList);
    if (pList.length === 0) {
    ...
    if (showEmpty) {
      return (
        <View style={styles.emptyWrap}>
          ...
          <Text style={styles.emptyInfo}>暂无降价喔，快去浏览商品吧~</Text>
        </View>
      );
    }
    return null;
              ...
              <TouchableOpacity activeOpacity={Layout.activeOpacity} onPress={this.cancelCollectProduct}>
                <View>
                  <LinearGradient style={styles.cancelCollect} start={{ x: 0, y: 0 }} end={{ x: 0, y: 0.48 }} colors={['#FF7BA1', '#FC4277']}>
                    <Text style={styles.collectBtn}>取消提醒</Text>
                  </LinearGradient>
                </View>
              </TouchableOpacity>
            </View>
