...
import {
  Image, StyleSheet, Text, View, TextInput, TouchableOpacity, ScrollView, Keyboard, StatusBar,
} from 'react-native';
import LinearGradient from 'react-native-linear-gradient';
import Toast from 'react-native-root-toast';
import { iosAppReviewSendSVC, iosAppReviewLogin } from '../../services/api';
...
export default class Login extends Component<Props> {
  ...
  async register() {
    ...
    const result = await iosAppReviewLogin({
      mobile,
      verificationCode,
    });
  ...
  async getVerificationCode() {
    ...
    const res = await iosAppReviewSendSVC(mobile);
    if (!res) {
      clearInterval(this.timer);
      this.setState({
        verBtnDisabled: false,
  ...
  render() {
    const {
      mobile, verBtnText, verBtnDisabled, verificationCode, inputFocus,
    } = this.state;
...
登录即表示同意米粒生活
          <Text onPress={() => this.props.navigation.navigate('WebView', { src: 'https://www.vxiaoke360.com/H5/mlsh-protocol/index.html' })} style={{ textDecorationLine: 'underline' }}>《用户服务协议》</Text>
        </Text>
      </ScrollView>
    );
  }
